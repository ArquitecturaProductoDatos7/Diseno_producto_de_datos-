...
class extrae_chunk_api_a_S3(luigi.Task)
    """
    Fetches a chunk of information from API to a S3 bucket.
    """
 ...
 class extrae_todo(luigi.Task):
    """
    Fetch all info from API to S3 (en formato csv)
    """
    
    def requires(self):
        chunk_size = 10 #10000     # el maximo que permite la API (o tamano del chunk)
        record = 1
        #nrecord = extrae.extrae_nhits(DATAURL) #total de records a extraer
        nrecord = 25
               
        bucket = maggiemusaetl
        root_path = demo
        etl_path = pruebas
        year = 2020
        fname = "file_" + str(file)
        
        file = 1
        nfile = int(np.ceil(nrecord/chunk_size)) # numero max de archivos/paginas que tendremos
        ...
        while record < nrecord:
            ...
            #mandamos a llamar a la tarea 1
            return extrae_chunk_api_a_S3(chunk_size, record, nrecord, bucket, root_path, etl_path, year, fname)
    ...
    def run(self):
        return print('llegue aqui')
    
    def output(self):
        return LuigiMock.MockFile("tarea_completa")


class extrae_todo2(luigi.Task):
    """
...
class extrae_info_api_a_local(luigi.Task):
    """
    Fetches all records to a JSON file.
    """
